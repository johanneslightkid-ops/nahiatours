        ...
        <div className="space-y-16">
          {platforms.map((platform) => (
            <div key={platform} className="space-y-6">
              {/* Platform header */}
              <div className={`inline-flex items-center gap-3 rounded-full border border-[rgba(150,112,31,0.4)] px-6 py-3 text-white shadow-oil-sm ${platformColors[platform].gradient}`}>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {videosByPlatform[platform].map((video, idx) => (
                  <div
                    key={video.id}
                    className="group relative rounded-2xl overflow-hidden shadow-oil-sm hover:shadow-oil-sm transition-all hover:-translate-y-2 border border-ink/20 hover:border-ink flex flex-col h-full bg-white"
                  >
                    {/* Video thumbnail/preview */}
                    <div className={`relative flex h-64 items-center justify-center overflow-hidden border-b border-[rgba(150,112,31,0.28)] ${platformColors[platform].gradient}`}>
                    ...
                    <div className="flex flex-1 flex-col p-5 bg-white space-y-4">
                      ...
                      <a
                        ...
                        className="mt-auto inline-flex w-full items-center justify-center rounded-full border border-[rgba(150,112,31,0.4)] bg-mango px-4 py-3 text-center font-extrabold text-ink shadow-oil-sm transition hover:-translate-y-0.5 hover:bg-mango-light"
                      >
            ...
            {platforms.map((platform) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`inline-flex items-center gap-2 rounded-full border border-[rgba(150,112,31,0.4)] px-6 py-3 font-extrabold text-white shadow-oil-sm transition hover:-translate-y-0.5 ${platformColors[platform].gradient}`}
                >
                  {platformIcons[platform]}
