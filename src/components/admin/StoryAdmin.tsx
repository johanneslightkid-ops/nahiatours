...
const StoryAdmin: React.FC = () => {
  ...
  const saveStory = async () => {
    if (!storyData.storyTitle.trim()) {
      setMessage({ type: 'error', text: 'Hace falta el título de la historia' });
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        setMessage({ type: 'error', text: 'No se pudo guardar la historia' });
      }
    } catch (error) {
  ...
  const handleImageUpload = async (elementId: string, file: File) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: '¡Imagen subida!' });
      setTimeout(() => setMessage(null), 2000);
    } catch (error) {
      setMessage({ type: 'error', text: 'No se pudo subir la imagen. Revisa la configuración de Cloudinary.' });
    ...
    return (
      <div key={element.id} className="space-y-3">
        {element.type === 'title' && (
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
          </div>
        )}
        ...
        {element.type === 'paragraph' && (
          <div>
            <label className="block text-xs font-semibold text-ink-soft mb-1">Texto</label>
            <textarea
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-sea-deep hover:bg-[#0f4f59] disabled:bg-sea text-white rounded-lg text-sm font-semibold transition"
              >
            ...
            <div>
              ...
              <input
                ...
                placeholder="Escribe el enlace de la imagen o súbela arriba"
                className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
            </div>
            ...
            {element.content.imageUrl && (
              <div className="mt-2 rounded-lg overflow-hidden border border-ink/15">
                <img
        ...
        {element.type === 'video' && (
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Título del video</label>
              <input
                type="text"
                value={element.content.title || ''}
                onChange={(e) => updateElement(element.id, 'title', e.target.value)}
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Fuente del video</label>
              <select
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Enlace del video</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Orientación</label>
              <div className="flex gap-2">
                <button
        ...
        {element.type === 'cta' && (
          <div className="space-y-4">
            {/* CTA Title */}
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Título del llamado a la acción</label>
              <input
                type="text"
                value={element.content.title || ''}
                onChange={(e) => updateElement(element.id, 'title', e.target.value)}
            ...
            {/* CTA Emoji/Icon */}
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Emoji/Icon</label>
              <input
                type="text"
            ...
            {/* CTA Description */}
            <div>
              <label className="block text-xs font-semibold text-ink-soft mb-1">Descripción</label>
              <textarea
                value={element.content.description || ''}
                onChange={(e) => updateElement(element.id, 'description', e.target.value)}
                placeholder="Escribe el texto del llamado a la acción"
                rows={3}
                className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
            </div>
            ...
            <div className="border-t border-ink/15 pt-4">
              <div className="flex justify-between items-center mb-3">
                <label className="block text-xs font-semibold text-ink-soft">Botones de acción</label>
                <button
                  onClick={() => {
                  ...
                  className="px-2 py-1 bg-teal-500 hover:bg-sea-deep text-white rounded text-xs font-semibold transition"
                >
                  + Add Button
                </button>
              </div>
              ...
              {element.content.buttons && element.content.buttons.length > 0 ? (
                <div className="space-y-3">
                  {element.content.buttons.map((button, btnIdx) => (
                    <div key={button.id} className="bg-paper p-3 rounded-lg space-y-2">
                      <div className="flex gap-2">
                        <input
                          type="text"
                          value={button.text}
                          ...
                          className="flex-1 rounded border border-ink/15 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-teal-500"
                        />
                        <button
                          onClick={() => {
                      ...
                      <input
                        ...
                        className="w-full rounded border border-ink/15 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-teal-500"
                      />
                ...
                <p className="text-xs text-ink-light italic">No buttons yet. Click "Add Button" to create one.</p>
              )}
            </div>
          </div>
        )}
  ...
  return (
    <div className="min-h-screen bg-paper-warm py-6 px-4 md:py-8">
      <div className="max-w-2xl mx-auto space-y-6">
        {/* Header */}
        <div className="bg-white rounded-3xl p-4 md:p-6 shadow-lg">
          <h1 className="text-2xl md:text-3xl font-bold text-ink mb-4">Historia de bienvenida</h1>
          ...
          <div className="flex gap-2 mb-4">
            <button
              ...
              className={`flex-1 px-4 py-2 rounded-lg font-semibold transition ${
                locale === 'en'
                  ? 'bg-sea-deep text-white'
                  : 'bg-paper-deep text-ink-soft hover:bg-slate-300'
              }`}
            >
              Inglés
            </button>
        ...
        <div className="bg-white rounded-3xl p-4 md:p-6 shadow-lg space-y-4">
          <h2 className="text-xl font-bold text-ink">Cabecera de la historia</h2>

          <div>
            <label className="block text-sm font-semibold text-ink-soft mb-2">Título de la historia</label>
            <input
              type="text"
              ...
              className="w-full rounded-lg border border-ink/15 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-ink-soft mb-2">Frase de la historia</label>
            <input
              type="text"
        ...
        {/* Add Element Buttons */}
        <div className="bg-white rounded-3xl p-4 md:p-6 shadow-lg">
          <h2 className="text-xl font-bold text-ink mb-4">Agregar elemento</h2>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-3">
            {(['title', 'paragraph', 'picture', 'video', 'cta'] as StoryElementType[]).map((type) => (
              <button
                key={type}
                onClick={() => addElement(type)}
                className="flex items-center justify-center gap-2 px-3 py-3 md:py-2 bg-sea-deep hover:bg-[#0f4f59] text-white rounded-lg font-semibold text-xs md:text-sm transition"
              >
                <FaPlus className="text-lg md:text-base" />
                <span className="hidden md:inline">{type === 'title' ? 'Título' : type === 'paragraph' ? 'Texto' : type === 'picture' ? 'Imagen' : type === 'video' ? 'Video' : 'CTA'}</span>
        ...
        <div className="bg-white rounded-3xl p-4 md:p-6 shadow-lg space-y-3">
          <h2 className="text-xl font-bold text-ink mb-4">Story Elements ({storyData.elements.length})</h2>

          {storyData.elements.length === 0 ? (
            <p className="text-ink-light text-center py-8">Todavía no hay elementos. ¡Agrega uno para empezar!</p>
          ) : (
            <div className="space-y-3">
              {storyData.elements.map((element, index) => (
                <div key={element.id} className="border border-ink/15 rounded-lg overflow-hidden">
                  {/* Element Header */}
                  <button
                    onClick={() =>
                      setExpandedElement(expandedElement === element.id ? null : element.id)
                    }
                    className="w-full px-4 py-3 md:py-4 bg-paper hover:bg-paper-warm transition flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <span className="text-xl">{elementTypeIcon(element.type)}</span>
                      <div className="text-left min-w-0 flex-1">
                        <p className="text-xs md:text-sm text-ink-light font-semibold">
                          #{index + 1} {element.type.toUpperCase()}
                        </p>
                        <p className="text-sm md:text-base font-semibold text-ink truncate">
                  ...
                  {/* Expanded Content */}
                  {expandedElement === element.id && (
                    <div className="border-t border-ink/15 px-4 py-4 space-y-4 bg-white">
                      {/* Element Editor */}
                      ...
                      <div className="flex flex-wrap gap-2 pt-4 border-t border-ink/15">
                        <button
                          ...
                          disabled={index === 0}
                          className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-blue-100 hover:bg-blue-200 disabled:bg-paper-warm text-blue-700 disabled:text-slate-400 rounded-lg text-sm font-semibold transition"
                        >
                          <FaArrowUp /> Up
                        </button>
                        <button
                      ...
                      <div className="pt-2 border-t border-ink/15">
                        <p className="text-xs font-semibold text-ink-soft mb-2">Agregar un elemento después de este</p>
                        <div className="grid grid-cols-2 gap-2">
                          {(['title', 'paragraph', 'picture', 'video', 'cta'] as StoryElementType[]).map((type) => (
                            ...
                            >
                              + {({ title: 'Título', paragraph: 'Texto', picture: 'Imagen', video: 'Video', cta: 'CTA' } as Record<string, string>)[type]}
                            </button>
                          ))}
                        </div>
                      </div>
        ...
        <button
          onClick={saveStory}
          disabled={isSaving}
          className="w-full bg-sea-deep hover:bg-[#0f4f59] disabled:bg-sea text-white px-6 py-4 rounded-2xl font-bold text-lg transition shadow-lg"
        >
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-xs md:text-sm text-blue-900">
          <p className="font-semibold mb-2">Configuración de JSONBin:</p>
          <p className="mb-2">Para los elementos de la historia en {locale === 'en' ? 'inglés' : 'español'}, asegúrate de tener esta variable de entorno:</p>
